            ...
            for server_message in stub.OpenChannel(iter(client_buffer.get, None)):
                ...
                if server_message.HasField("downloadOrder"):
                    ...
                    if download_order.ifTrain == 0:
                        ...
                        filepath = f"classify/grpc_transfer/client/clientDB/client{num}/{filename}"
                        with open(filepath, 'wb') as file:
                            file.write(filedata)
                        os.remove(f"classify/grpc_transfer/client/clientDB/client{num}/mnist.pt")
                        fin += 1
                        print("Done")
                        if fin == 3:
                            flag = 0
                ...
                if server_message.HasField("uploadOrder"):
                    upload_order_message = server_message.uploadOrder
                    filename = upload_order_message.name
                    filepath = "client1/" + filename
                    with open(filepath, "rb") as file:
                        filedata = file.read()
                    upload_acknowledgement_message = UploadAcknowledgement(name = filename, data = filedata)
                    message_to_server = ClientMessage(uploadAcknowledgement = upload_acknowledgement_message)
                    client_buffer.put(message_to_server)
