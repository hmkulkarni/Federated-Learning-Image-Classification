...
import sys
sys.path.insert(1, "classify/grpc_transfer/")
...
class FileTransferServicer(fileTransfer_pb2_grpc.FileTransferServicer):
    def OpenChannel(self, request_iterator, context):
        ...
        global rounds, completed_clients, device
        ...
        filepath = f"classify/grpc_transfer/server/serverDB/CR{rounds}/" + filename
        ...
        num = len(os.listdir(f"classify/grpc_transfer/server/serverDB/CR{rounds + 1}/"))
        
        num_files = len(os.listdir(f'classify/grpc_transfer/server/serverDB/CR{rounds + 1}/'))
        
        # If there are less than 3 files, then save the file and continue
        if num_files < 3:
            torch.save(trained_model, f"classify/grpc_transfer/server/serverDB/CR{rounds + 1}/mnist{num + 1}.pt")
        
        num_files = len(os.listdir(f'classify/grpc_transfer/server/serverDB/CR{rounds + 1}/'))
        
        # If there are 3 files in the CR folder, then perform federated average
        if num_files == 3:
            m1 = torch.load(f"classify/grpc_transfer/server/serverDB/CR{rounds + 1}/mnist1.pt")
            m2 = torch.load(f"classify/grpc_transfer/server/serverDB/CR{rounds + 1}/mnist2.pt")
            m3 = torch.load(f"classify/grpc_transfer/server/serverDB/CR{rounds + 1}/mnist3.pt")
            l = [m1, m2, m3]
        
            m = LeNetMnist().to(device)
            avg = averageWeights(l)
            if rounds < 2:
                torch.save(avg, f"classify/grpc_transfer/server/serverDB/CR{rounds + 1}/mnist.pt")
                m.load_state_dict(torch.load(f"classify/grpc_transfer/server/serverDB/CR{rounds + 1}/mnist.pt"))
            elif rounds == 2:
                torch.save(avg, f"classify/grpc_transfer/server/serverDB/CR{rounds + 1}/final_mnist.pt")
                m.load_state_dict(torch.load(f"classify/grpc_transfer/server/serverDB/CR{rounds + 1}/final_mnist.pt"))
            ...
            with open("classify/grpc_transfer/server/serverDB/averageWeights.txt", "a") as f:
